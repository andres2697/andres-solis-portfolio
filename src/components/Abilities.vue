<script setup>
  ...
  // Variables
  const containerOff = 'absolute w-[40%] h-[70%] bg-white bg-opacity-85 opacity-30 rounded-md shadow-xl shadow-gray-700'
  const containerOn = 'absolute w-[40%] h-[70%] bg-white bg-opacity-85 opacity-100 rounded-md shadow-xl shadow-gray-700'
  const positions = ref([
    { clase: 'top-[5%] left-[30%] transform'  },
    { clase: 'top-[25%] left-[85%] transform transition-right' },
    ...
    { clase: 'top-[25%] left-[-25%] transform transition-left' }
  ])
  const content = abilitiesList
  const regexFirst = /top-\[5%\] left-\[30%\]./
  const regexLeft = /top-\[25%\] left-\[-25%\]./
  const regexRight = /top-\[25%\] left-\[85%\]./
  ...
  <div class="w-full h-screen flex flex-col justify-center items-center align-middle bg-gradient-to-r from-gray-700 via-gray-600 to-gray-500">
    <div class="w-full h-[15%] flex flex-col justify-center items-center text-center p-5 rounded-md mt-5">
      <h1 class="w-[60%] flex items-center justify-center py-2 text-black text-4xl font-bold bg-white bg-opacity-90 rounded-md shadow-md shadow-gray-700">
        <font-awesome-icon icon="gear" class="mt-auto mb-auto mr-5 px-3 py-1 text-2xl font-semibold"/>
        Mis habilidades
      </h1>
    </div>
    <div class="w-full relative h-[85%] flex flex-col justify-center items-center overflow-hidden">
      <div class="absolute inset-0 flex flex-col justify-center items-center w-fit h-fit ml-10 text-center">
        <div
          class="flex justify-center items-center bg-white w-10 h-10 rounded-full text-3xl text-gray-700 font-bold p-6 hover:cursor-pointer shadow-md shadow-gray-700"
          @click="spinAbilities"
          @mouseover="spin = true"
      ...
      >
        <h3 class="flex w-full text-center justify-center items-center h-[15%] font-bold text-xl mt-2">
          {{ content[index].title }}
        </h3>
        <div class="grid grid-rows-4 grid-flow-col gap-4 w-full h-[85%] mt-auto ml-auto mr-4 pl-6 pr-6 text-start">
          <div v-for="(ability, index) in content[index].abilities.ability" :key="index" class="flex align-center items-center">
